...
export class BuildMenuRenderer {
  ...
  constructor(container: HTMLElement) {
    ...
    optionsRow.style.cssText = 'display: flex; gap: 12px;';
    ...
    for (const entry of entries) {
      ...
      opt.style.cssText = `
        ...
        white-space: nowrap;
      `;
      opt.textContent = `${entry.key}: ${def.name} (${def.costMinerals}m)`;
  ...
  update(visible: boolean, minerals: number, _gas: number, selectedType: number = 0): void {
    ...
    const costs = [
      BUILDING_DEFS[BuildingType.CommandCenter].costMinerals,
      BUILDING_DEFS[BuildingType.SupplyDepot].costMinerals,
      BUILDING_DEFS[BuildingType.Barracks].costMinerals,
    ];
